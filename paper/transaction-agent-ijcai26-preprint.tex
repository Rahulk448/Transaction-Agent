...
by pausing the transaction and collecting additional evidence. We present an
extended research prototype of a transaction-decision agent that (i)~summarizes a
...
legitimate/fraudulent state, (iv)~tracks belief uncertainty via Shannon entropy $H(P)$ and belief shift via Kullback-Leibler divergence $D_{\text{KL}}$, (v)~derives cost-optimal decision boundaries ($p^* = 44.4\%$), and (vi)~dynamically selects verification channels based on Expected Information Gain per unit cost ($\text{EIG}/\text{Cost}$). We evaluate the agent on a frozen, ground-truth-blind benchmark of 35 cases, reporting an Expected Calibration Error ($\text{ECE} = 0.1283$), mean posterior entropy ($\bar{H} = 0.5365$~bits), and mean KL divergence ($\bar{D}_{\text{KL}} = 0.3862$~bits). The system is implemented in pure Python (standard library only; 34 passing unit and integration tests).
\end{abstract}
...
\subsection{Information-Theoretic Metrics and Active Information Selection}

Beyond Bayesian updating and expected cost minimization, the Week~2 extension endows the agent with quantitative information-theoretic tracking and active sensing:

\paragraph{Shannon Entropy.}
At any belief state $P = [p_{\text{legit}}, p_{\text{fraud}}]$, the agent quantifies its doubt in bits using Shannon entropy:
\begin{equation}
H(P) = -\sum_{x \in \{\text{legit}, \text{fraud}\}} P(x) \log_2 P(x)
\end{equation}
A uniform prior $P=[0.5, 0.5]$ yields $H(P)=1.0$~bit (maximum uncertainty), while a confident prior $P=[0.9, 0.1]$ yields $H(P) \approx 0.469$~bits.

\paragraph{Kullback-Leibler Divergence.}
To measure the magnitude of belief shift (or surprise) induced by evidence $E$, the agent computes the relative entropy between posterior $Q$ and prior $P$:
\begin{equation}
D_{\text{KL}}(Q \parallel P) = \sum_{x} Q(x) \log_2 \left( \frac{Q(x)}{P(x)} \right)
\end{equation}
If evidence provides zero informative signal ($Q=P$), $D_{\text{KL}} = 0.0$~bits; strong signals produce high KL divergence, surfacing anomalous transactions for auditability.

\paragraph{Derived Decision Boundaries.}
Rather than hardcoding decision thresholds, the agent derives the optimal binary break-even threshold $p^*$ from the cost model:
\begin{equation}
p^* = \frac{C_{\text{FP}}}{C_{\text{FP}} + C_{\text{FN}}} = \frac{8.0}{8.0 + 10.0} = 44.4\%
\end{equation}
When \textsc{Hold} is available at friction cost $C_{\text{hold}}=2.0$, solving $EC(\textsc{Approve}) = EC(\textsc{Hold})$ and $EC(\textsc{Hold}) = EC(\textsc{Stop})$ yields lower bound $p^*_{\text{low}} = 20.0\%$ and upper bound $p^*_{\text{high}} = 75.0\%$.

\paragraph{Active Sensing via Information Efficiency.}
When entering the uncertain region, the agent actively selects among candidate verification channels (\texttt{push\_auth}, \texttt{sms\_otp}, \texttt{manual\_review}) by computing Expected Information Gain $\text{EIG}(V) = H(P) - \mathbb{E}[H(Q \mid V)]$ and prioritizing the channel maximizing Information Efficiency:
\begin{equation}
\text{Efficiency}(V) = \frac{\text{EIG}(V)}{\text{Cost}(V)}
\end{equation}
This ensures high-cost interventions (such as manual review at cost $10.0$) are rejected when lightweight push or SMS checks offer superior uncertainty reduction per unit cost.
...
\subsection{Information-Theoretic and Calibration Evaluation}

Evaluating the agent across the 35 benchmark cases yields quantitative metrics for calibration reliability, residual uncertainty, and belief dynamics:

\begin{itemize}
\item \textbf{Expected Calibration Error (ECE):} $\text{ECE} = 0.1283$. Across probability bins, the agent's assigned fraud probabilities closely track empirical fraud occurrences, with slight overconfidence in mixed-signal cases under conditional independence.
\item \textbf{Mean Posterior Entropy ($\bar{H}$):} $\bar{H} = 0.5365$~bits across all evaluated cases, demonstrating that the agent successfully destroys significant uncertainty relative to the maximum $1.0$~bit uniform state.
\item \textbf{Mean KL Divergence ($\bar{D}_{\text{KL}}$):} $\bar{D}_{\text{KL}} = 0.3862$~bits, quantifying the average information update imparted by incoming feature signals over the baseline prior.
\item \textbf{Test Suite Verification:} The expanded codebase is validated by 34 passing unit and integration tests covering Shannon entropy, KL divergence, derived cost boundaries, active sensing EIG selection, and ECE calculation.
\end{itemize}
